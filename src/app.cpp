...
std::unordered_map<unsigned int, Character>& App::get_characters() {
  return characters;
}

std::unordered_map<unsigned int, Resource>& App::get_resources() {
  return resources;
}

std::vector<Group>& App::get_groups() {
  return groups;
}

std::vector<std::shared_ptr<UniversalEntity>>& App::get_universal_entities() {
...
void App::draw_commands() const {
  ...
  int selected_character_count = std::accumulate(characters.begin(), characters.end(), 0, [](int acc, const auto& kv) {
    return acc + (kv.second.is_selected() ? 1 : 0);
  });
...
void App::draw_resources() const {
  int offset = 10;

  for (int i = 0; i < RESOURCE_COUNT; i++) {
    const char* label = TextFormat("%s: %d | ", RESOURCE_NAMES[i], groups[PLAYER_CHARACTER_GROUP].resource_amounts[i]);
...
void App::update_map_drag() {
  if (IsMouseButtonDown(2)) {
    camera.offset = Vector2Add(camera.offset, GetMouseDelta());
  }
}

void App::update_character_resource_harvest_initialization() {
...
const Resource* App::resource_at_pos(Vector2 pos) const {
...
void App::delete_automations_for_character(const unsigned int character_id) {
